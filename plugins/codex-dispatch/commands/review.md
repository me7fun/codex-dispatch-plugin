...
2. 呈現：
   ...
   - `reason=reviewer-claude`（輸出以「○ review：reviewer=claude」開頭）：本專案設定不用 Codex。用 `Agent`（Explore 唯讀）跑 `${CLAUDE_PLUGIN_ROOT}/prompts/self-review.md` 的 **A. 審 diff** 變體，`{{TARGET}}`／`{{ROOT}}` 照輸出的 `Target:`／`Root:` 行填、`{{FOCUS}}` 填 `$ARGUMENTS` 的 focus；結果照 findings 格式呈現。仍然不自動修、不記未審清單、不加任何標題。
